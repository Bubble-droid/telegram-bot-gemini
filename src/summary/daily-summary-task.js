...
import { getJsonFromKv, putJsonToKv, sendErrorNotification } from '../utils';
...
async function sendSummaryNotification(env, sendTelegramMessage, message) {
	...
	const maintainerUserIds = maintainerUserIdsString.split(",").map(id => parseInt(id.trim())).filter(id => !isNaN(id));
...
async function sendSummaryToGemini(env, groupHistory, sendTelegramMessage) {
	...
	try {
		...
		const response = await fetch(geminiApiUrl, { // 使用 fetch 发送 HTTP POST 请求
			method: 'POST',
			headers: {
		...
		await sendErrorNotification(env, error, "src/summary/daily-summary-task.js - sendSummaryToGemini 函数 - 调用 Gemini API 进行总结失败", sendTelegramMessage);
		return null;
	}
}
