...
import { getJsonFromKv, handleCooldownReplyAndCleanup } from '../utils';
...
import { handleCommandReplyAndCleanup, recordBotReplyMessage } from './command-handler'; //  !!!  导入 handleCommandReplyAndCleanup  !!!
 ...
 * @param {KVNamespace} taskQueueKv  任务队列 KV 命名空间  !!!  新增 taskQueueKv 参数  !!!
 * @returns {Promise<Response>}
 */
export async function handleSearchCommand(message, env, botName, sendTelegramMessage, defaultModelName, deleteTelegramMessage, taskQueueKv) { //  !!!  新增 deleteTelegramMessage, taskQueueKv 参数 !!!
	...
	//  检查搜索关键词是否为空 (保持不变)
	if (!searchQuery) {
		const replyText = `🤔 <b>请输入您要搜索的内容。</b>\n\n` +
			`例如：<code>/search@${botName} 最新 Sing-box 教程</code>\n\n` +
			`请在 <code>/search@${botName}</code> <b>命令后添加您的搜索关键词。</b>`; //  !!!  更新提示信息，包含 /search 命令用法  !!!
	...
	if (isInSearchCooldown) {
		...
		//  !!!  调用 handleCommandReplyAndCleanup 发送冷却提示 (从 command-handler.js 导入) !!!
		await handleCooldownReplyAndCleanup(env.BOT_TOKEN, groupId, replyText, message.message_id, sendTelegramMessage, deleteTelegramMessage, env, message.message_id, botName); //  !!!  调用 handleCommandReplyAndCleanup  !!!
		return new Response('OK');
	}
	...
	try {
		...
		let systemSearchInstructionData = await getJsonFromKv(systemInitConfigKv, systemSearchPromptKey) || { systemSearchPrompt: "You are a helpful assistant, who answers questions using google search." }; //  获取搜索提示词
