...
import { addGroupToWhitelist, isUserWhitelisted, removeGroupFromWhitelist, putJsonToKv, getJsonFromKv } from '../utils'; //  导入白名单管理函数
import { clearGroupContextHistory, clearUserContextHistory } from '../storage/context-storage'; //  导入上下文清理函数
...
export async function handleBotCommand(message, env, botName, sendTelegramMessage, defaultModelName, deleteTelegramMessage, taskQueueKv) { //  !!!  修正后的函数定义 !!!
	...
	if (message.entities) { //  优先使用 entities 检测
		for (const entity of message.entities) {
			if (entity.type === 'bot_command') {
				...
				if (normalizedCommandText.startsWith(botCommandPrefix) && normalizedCommandText.includes(botNameMentionLowerCase)) { //  !!!  检查命令是否以 / 开头 并且 包含 @botname (忽略大小写) !!!
	...
	if (!isBotCommand) { //  如果 entities 中没有 bot_command，则 Fallback 到旧的检测逻辑 (已移除, 这里为了代码结构保留)
		//  !!!  此处已移除旧的命令检测 Fallback 逻辑，因为 index.js 已经做了基于 entities 的命令检测 !!!
		console.log("handleBotCommand: 消息不是 Bot 命令 (即使 entities 中有 bot_command 类型，但解析失败)，忽略"); //  更明确的日志
		return new Response('OK'); //  不是 Bot 命令，直接返回
	}
	...
	if (chatType === 'private') { //  !!!  私聊消息中，忽略所有命令 !!!
	...
	} else if (command === 'clear_user_context') { //  !!!  /clear_user_context 命令的特殊权限处理 !!!
		const groupWhitelistKey = env.GROUP_WHITELIST_KV_KEY;
		const groupWhitelist = await getJsonFromKv(botConfigKv, groupWhitelistKey) || [];
		if (groupWhitelist.includes(groupId)) {
	...
	switch (command) {
		case 'start':
			replyText = `👋 <b>您好，我是 Athena 助手！</b>\n\n` + //  更友好的欢迎语，加粗
				...
				`<b>⚠️ 格式规范：</b>为了确保我能正确理解您的指令，请<b>务必使用规范的命令和 @ 格式</b>，例如：<code>/start@${botName}</code>， <code>/search@${botName} 关键词</code>，以及直接 <code>@${botName} 提问内容</code>。 避免在命令或 @ 符号中使用特殊字符或空格。\n\n` + //  !!!  强调规范格式的重要性  !!!
		...
		case 'clear_user_context': //  !!!  新增 clear_user_context 命令处理 !!!
			...
			//  !!!  传递 botConfigKv 参数 !!!
			await handleCommandReplyAndCleanup(env.BOT_TOKEN, groupId, replyText, message.message_id, sendTelegramMessage, deleteTelegramMessage, env, message.message_id, botName); //  !!!  传递 env.TASK_QUEUE_KV !!!
			return new Response('OK');
		...
		default:
			...
			try {
				...
				await sendErrorNotification(env, error, "command-handler.js - handleBotCommand 函数 - default case - handleCommandReplyAndCleanup 执行失败", sendTelegramMessage);
			}
			return new Response('OK');
	}
}
 ...
 */
export async function handleCommandReplyAndCleanup(botToken, chatId, replyText, commandMessageId, sendTelegramMessage, deleteTelegramMessage, env, replyToMessageId = null, botName) { //  !!!  新增 taskQueueKv 参数 !!!
 ...
 * @returns {Promise<void>}
 */
export async function recordBotReplyMessage(env, botName, replyText, groupId) { //  !!!  修改为 env 参数 !!!
